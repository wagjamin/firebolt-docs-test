...
sidebarTitle: GEOGRAPHY data type
...
Firebolt supports creating `GEOGRAPHY` from the industry standard GeoJSON, Well-Known Text (WKT), and Well-Known Binary (WKB) representations, as well as the extended formats EWKT and EWKB introduced by the PostGIS extension. See the corresponding [function documentations](/reference-sql/functions-reference/geospatial) for further detail.
...
The cast from `BYTEA` to `GEOGRAPHY` behaves exactly like the [ST_GEOGFROMWKB](/reference-sql/functions-reference/geospatial/st_geogfromwkb) function.

### Cast to BYTEA

The cast from `GEOGRAPHY` to `BYTEA` behaves exactly like the [ST_ASEWKB](/reference-sql/functions-reference/geospatial/st_asewkb) function.

### Cast from TEXT

The cast from `TEXT` to `GEOGRAPHY` automatically detects the decoding used and supports the [Extended Well-Known Text (EWKT)](https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry), [GeoJSON](https://datatracker.ietf.org/doc/html/rfc7946), and hex-encoded [Extended Well-Known Binary (EWKB)](https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry#Well-known_binary) formats. It behaves exactly like [ST_GEOGFROMTEXT](/reference-sql/functions-reference/geospatial/st_geogfromtext) for [Well-Known Text (WKT)](https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry) inputs, like [ST_GEOGFROMGEOJSON](/reference-sql/functions-reference/geospatial/st_geogfromgeojson) for [GeoJSON](https://datatracker.ietf.org/doc/html/rfc7946) inputs, and like [ST_GEOGFROMWKB](/reference-sql/functions-reference/geospatial/st_geogfromwkb)[(DECODE(input, 'HEX'))](/reference-sql/functions-reference/bytea/decode) for hex-encoded [Well-Known Binary (WKB)](https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry#Well-known_binary) inputs.

### Cast to TEXT

The cast from `GEOGRAPHY` to `TEXT` returns the hexadecimal representation of the output of the [ST_ASEWKB](/reference-sql/functions-reference/geospatial/st_asewkb) function in upper case letters.
...
- [Python](/guides/developing-with-firebolt/connecting-with-Python) - version 1.6.0.
- [JDBC](/guides/developing-with-firebolt/connecting-with-jdbc) - version 3.4.0.
- [Node.js](/guides/developing-with-firebolt/connecting-with-nodejs) - any version.
- [Go](/guides/developing-with-firebolt/connecting-with-go) - version 1.3.1.
- [.NET](/guides/developing-with-firebolt/connecting-with-net-sdk) - version 1.4.0.
