...
Returns an array containing all the elements that are present in every argument array. It uses set semantics, which means the result cannot contain multiple copies of the same value. The order of elements in the result may be different than in the original arrays; use [ARRAY_SORT](/reference-sql/functions-reference/array/array-sort) to stipulate a specific order on the results.
