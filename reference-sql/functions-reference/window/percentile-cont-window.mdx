...
PERCENTILE\_CONT is available as an Window function. See also [PERCENTILE\_DISC](/reference-sql/functions-reference/window/percentile-disc-window), which returns a percentile over a partition equal to a specific column value. For more information on usage, please refer to [Window Functions](/reference-sql/functions-reference/window).
