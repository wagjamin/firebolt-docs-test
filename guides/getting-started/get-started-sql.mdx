...
* An [engine](/overview/engine-fundamentals) provides the compute resources for ingesting data and running queries. For more information on using Firebolt engines and how to select the correct size for your workload, see [Operate engines](/guides/operate-engines).
...
After creating an engine, you can load your data. This tutorial uses Firebolt's publicly available Firebolt’s sample dataset, from the fictional [“Ultra Fast Gaming Inc.”](https://help.firebolt.io/t/ultra-fast-gaming-firebolt-sample-dataset/250) company. This dataset does not require access credentials. If your personal dataset requires access credentials, you will need to provide them. For examples of how to provide access credentials and more complex loading workflows, see [Loading data](/guides/loading-data). For more information about AWS access credentials, see [Creating Access key and Secret ID](/guides/loading-data/creating-access-keys-aws)

<Note>
If you used the **Load data** wizard, skip ahead to the following **Run query** section.
</Note>

Use [COPY FROM](/reference-sql/commands/data-management/copy-from) in the **Develop Space** to copy data directly from a source into a Firebolt managed table.
...
1. Enter the following  into the **Script 1** tab to load data using the following steps:
  ...
  ```
  For examples of more complex loading workflows, see [Load data](/guides/loading-data).
...
For more information about Firebolt’s **Develop Space**, see [Using the develop workspace](/guides/query-data/using-the-develop-workspace).
...
For more information about Firebolt’s **Develop Space**, see [Use the Develop Space](/guides/query-data/using-the-develop-workspace). 
...
When data is warm, Firebolt transfers data from remote storage in Amazon (S3) to a local (cache). Data is automatically warmed when you access it during a query, and stored in a solid state drive (SSD) cache. However, when you query data to warm it, you use an engine, and incur [engine consumption](/overview/engine-fundamentals/engine-consumption) costs. Therefore, you should use filters to warm only the data that you need to access frequently in your queries.
...
To remove a table and all of its data, enter [DROP TABLE](/reference-sql/commands/data-definition/drop-table) into a script tab, as shown in the following code example:
``` sql
DROP TABLE levels
```
...
If you want to save your data outside of Firebolt, you can use [COPY TO](/reference-sql/commands/data-management/copy-to) to export data to an external table. This section shows how to set the minimal AWS permissions and use `COPY TO` to export data to an [AWS S3 bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Welcome.html). You may have to reach out to your administrator to obtain or change AWS permissions.
...
2. An AWS IAM policy statement attached to a user role. Firebolt requires the following minimum permissions in the IAM policy:
    ...
    For more information about AWS access keys and roles, see [Creating Access Key and Secret ID in AWS](/guides/loading-data/creating-access-keys-aws).

### Export to an AWS bucket

Use [COPY TO](/reference-sql/commands/data-management/copy-to) select all the columns from a table and export to an AWS S3 bucket as shown in the following code example:
