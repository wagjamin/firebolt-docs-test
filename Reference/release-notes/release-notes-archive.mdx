...
[APACHE_DATASKETCHES_HLL_BUILD](/sql_reference/functions-reference/datasketches/apache-datasketches-hll-build),
[APACHE_DATASKETCHES_HLL_MERGE](/sql_reference/functions-reference/datasketches/apache-datasketches-hll-merge),
and [APACHE_DATASKETCHES_HLL_ESTIMATE](/sql_reference/functions-reference/datasketches/apache-datasketches-hll-estimate).
...
Firebolt now supports the `ntile` window function. Refer to our [NTILE](/sql_reference/functions-reference/window/ntile) documentation for examples and usage. 
...
Removed support for `json_extract_raw`, `json_extract_array_raw`, `json_extract_values`, and `json_extract_keys`. Updated `json_extract` function: the third argument is now `path_syntax`, which is a JSON pointer expression. See [JSON_EXTRACT](/sql_reference/functions-reference/JSON/json-extract) for examples and usage. 

 **Cluster ordinal update**

Replaced `engine_cluster` with [cluster_ordinal](/sql_reference/information-schema/engine-metrics-history) in `information_schema.engine_metrics_history`. The new column is an integer representing the cluster number.

 **Configurable cancellation behavior on connection drop**

Introduced the `cancel_query_on_connection_drop` setting, allowing clients to control query cancellation on HTTP connection drop. Options include `NONE`, `ALL`, and `TYPE_DEPENDENT`. Refer to [system settings](/system-settings#query-cancellation-mode-on-connection-drop) for examples and usage. 
...
* [Resolved issues](/release-notes#resolved-issues)
...
* [Enhancements, changes, and new integrations](/release-notes#enhancements-changes-and-new-integrations)
* [Breaking Changes](/release-notes#breaking-changes)
...
The `match` function has been removed and replaced with [regexp_like](/sql_reference/functions-reference/string/regexp-like). 
...
The [array_first](/sql_reference/functions-reference/Lambda/array-first) function has been added. It returns the first element in the given array for which the given function returns `true`.

**New name for `any_match`**

A new name for `any_match` has been added: [array_any_match](/sql_reference/functions-reference/Lambda/array-any-match). `any_match` will be kept as an alias. 
...
* [New features](/release-notes#new-features)
* [Enhancements, changes, and new integrations](/release-notes#enhancements-changes-and-new-integrations)
* [Resolved issues](/release-notes#resolved-issues)
...
* Fixed a bug involving ['btrim'](/sql_reference/functions-reference/string/btrim) string characters, where invoking `btrim`, `ltrim`, `rtrim`, or `trim` with a literal string but non-literal trim characters could result in an error.
...
Support has been added to allow the [ARRAY_ANY_MATCH](/sql_reference/functions-reference/Lambda/array-any-match) lambda function to work with nullable arrays.
...
Changed the behavior of [split_part](/sql_reference/functions-reference/string/split-part) when an empty string is used as delimiter.
...
[New comparison operators](/sql_reference/operators) `IS DISTINCT FROM` and `IS NOT DISTINCT FROM` have been added.
...
A new alias [ARRAY_TO_STRING](/sql_reference/functions-reference/array/array-to-string) has been added to function `ARRAY_JOIN`.
