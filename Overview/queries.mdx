---
description: Learn about running queries, improving performance, and query optimizations
  in Firebolt.
mode: wide
...
* [Spilling intermediate query state](/queries/understand-spilling) to the local SSD cache. This allows processing queries whose working set exceeds main memory.
...
* [Reusing query sub-results and result caching](/queries/understand-query-performance-subresult) to reduce redundant calculations across queries.
* [History-based query optimization](/queries/understand-query-performance-hbs), which leverages past query patterns to improve query plans for new queries.
