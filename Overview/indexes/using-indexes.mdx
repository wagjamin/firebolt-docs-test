...
Topics:
- [Data modeling](/indexes#data-modeling)
  - [How it works](/indexes#how-it-works)
    - [Databases](/indexes#databases)
      - [Create a database](/indexes#create-a-database)
      - [Manage a database](/indexes#manage-a-database)
      - [Database best practices](/indexes#database-best-practices)
        - [Database best practice example](/indexes#database-best-practice-example)
      - [Evaluate your database for performance](/indexes#evaluate-your-database-for-performance)
    - [Schema](/indexes#schema)
      - [Schema best practices](/indexes#schema-best-practices)
    - [Tables](/indexes#tables)
      - [Firebolt-managed tables](/indexes#firebolt-managed-tables)
      - [External tables](/indexes#external-tables)
      - [Editing and deleting tables](/indexes#editing-and-deleting-tables)
      - [Primary indexes in tables](/indexes#primary-indexes-in-tables)
        - [Create a primary index](/indexes#create-a-primary-index)
        - [Primary index best practices](/indexes#primary-index-best-practices)
      - [Aggregating indexes in tables](/indexes#aggregating-indexes-in-tables)
        - [Create an aggregating index](/indexes#create-an-aggregating-index)
        - [Best practices for aggregate indexes](/indexes#best-practices-for-aggregate-indexes)
      - [Partitions in tables](/indexes#partitions-in-tables)
        - [Create a partition](/indexes#create-a-partitioned-table)
      - [Suggested indexes and partitions](/indexes#suggested-indexes-and-partitions)
    - [Additional resources](/indexes#additional-resources)
...
* [Create a database](/indexes#create-a-database) &ndash; Use a system or user engine to create a database.
* [Manage a database](/indexes#manage-a-database) &ndash; How to edit and delete databases.
* [Database best practices](/indexes#database-best-practices) &ndash; How to organize your databases for the best performance.
* [Evaluate your database for performance](/indexes#evaluate-your-database-for-performance).

#### Create a database

Use [CREATE DATABASE](/sql_reference/commands/data-definition/create-database), which requires only the name of the database, and an **engine** to create a database.
...
You can delete a database with [DROP DATABASE](/sql_reference/commands/data-definition/drop-database), which permanently deletes the database and all its contents. Since this action cannot be undone, use it carefully. Create a backup before dropping the database to prevent data loss.
...
Table topics:
* [Firebolt-managed tables](/indexes#firebolt-managed-tables) &ndash; Fact and dimension tables make the best use of Firebolt's optimization strategies.
* [External tables](/indexes#external-tables) &ndash; Users can access and query data without loading it into Firebolt. External tables generally have poorer performance compared to Firebolt-managed tables.
* [Editing and deleting tables](/indexes#editing-and-deleting-tables) &ndash; You can edit and delete an existing table.
* [Primary indexes in tables](/indexes#primary-indexes-in-tables) &ndash; Select the most efficient primary index for your tables based on your query patterns and data characteristics.
* [Aggregating indexes in tables](/indexes#aggregating-indexes-in-tables) &ndash; Precompute and update aggregation results for fast calculations and to save compute resources.
* [Partitions in tables](/indexes#partitions-in-tables) &ndash; Organize data based on date ranges, regions, customer types or other categories.
* [Suggested indexes and partitions](/indexes#suggested-indexes-and-partitions) &ndash; Use Firebolt's [RECOMMEND_DDL](/sql_reference/commands/queries/recommend_ddl) tool to suggest indexes and partitions based on your query history.
...
```
In the previous example, [CREATE TABLE](/sql_reference/commands/data-definition/create-fact-dimension-table) creates a fact table by default, and the primary index acts as a composite index of both the `book_id` and `checkout_date` columns.
...
The following code example shows how to create an aggregating index to precompute the number of transactions per borrower and their average late fee on the existing `borrowedbooks` table created in the previous **Fact tables** section in [Firebolt managed-tables](/indexes#firebolt-managed-tables):
...
The following code example uses `RECOMMEND_DDL` to analyze query patterns on the books table, created in the **Dimension tables ** section under [Firebolt managed-tables](/indexes#firebolt-managed-tables), based on queries run in the past week:
...
* [RECOMMEND_DDL](/sql_reference/commands/queries/recommend_ddl) &ndash; Information on syntax, parameters and examples of using Firebolt’s tool to automatically recommend optimal primary index and partition strategies.
