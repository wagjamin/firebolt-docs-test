---
description: Learn how to view and verify assigned privileges for users and roles
  in Firebolt.
mode: wide
sidebarTitle: Check Assigned Privileges
...
In the following code example, a user `test_user` with an [account_admin](/Overview/organizations-accounts#account-administrative-role) role retrieves their privileges and associated roles for their current user session:
