---
description: Understand the permissions that can be assigned at the database level
  in Firebolt, including controlling access to database objects and managing database-level
  operations.
mode: wide
...
| [VACUUM](/sql_reference/commands/data-management/vacuum) ANY             | Allows running the `VACUUM` operation on all current and future tables.              | `GRANT VACUUM ANY ON DATABASE <database_name> TO <role>;`              | `REVOKE VACUUM ANY ON DATABASE <database_name> FROM <role>;`           |
...
### USAGE permission
The following code example [grants](/sql_reference/commands/access-control/grant) the role `developer_role` access to use the specified database:
...
The following code example gives the role `developer_role` permission to run [VACUUM](/sql_reference/commands/data-management/vacuum) operations on all current and future tables in the specified database:
