...
You can use `LOCATION` to centralize credential storage with managed access with Role-Based Access Control ([RBAC](/Overview/Security/Role-Based Access Control)) for `CREATE`, `MODIFY`, and `USAGE` permissions for different users, use a single location definition for multiple tables and queries, and control access to sensitive data. `LOCATION` is version control-friendly because no sensitive credentials are stored in source code.

After you create a `LOCATION`, you can use the `information_schema.locations` view to see [details about all the locations](/sql_reference/information-schema/locations) in your account including source type, URL, description, owner and creation time.

 **Topics:**
 * [Syntax](/commands/data-definition#syntax)
 * [Parameters](/commands/data-definition#parameters)
 * [Specify credentials to access Amazon S3](/commands/data-definition#specify-credentials-to-access-amazon-s3)
 * [Permissions](/commands/data-definition#permissions)
 ...
 * [Limitations](/commands/data-definition#limitations)
...
`LOCATION` objects are managed using [RBAC](/Overview/Security/Role-Based Access Control) with the following permission levels:
...
* [Authenticate using an access key](/commands/data-definition#authenticate-using-an-access-key)
* [Authenticate using a role](/commands/data-definition#authenticate-using-a-role)
* [Create a location with a description](/commands/data-definition#create-a-location-with-a-description)
* [Create a location with an AWS session token](/commands/data-definition#create-a-location-with-an-aws-session-token)
* [Create a location only if it doesn't exist](/commands/data-definition#create-a-location-only-if-it-doesnt-exist)
* [Use a location to load data into an external table](/commands/data-definition#use-a-location-to-load-data-into-an-external-table)
* [Use a location to load data using COPY statements](/commands/data-definition#use-a-location-to-load-data-using-copy-statements)
* [Use a location to load data with a TVF](/commands/data-definition#use-a-location-to-load-data-with-a-tvf)
* [Alter a location](/commands/data-definition#alter-a-location)
* [Delete a location](/commands/data-definition#delete-a-location)
...
For more information about using locations in external tables, see [CREATE EXTERNAL TABLE](/sql_reference/commands/data-definition/create-external-table).
...
For more information, see [COPY TO](/sql_reference/commands/data-management/copy-to) and [COPY FROM](/sql_reference/commands/data-management/copy-from).

### Use a `LOCATION` to load data with a TVF

You can use `LOCATION` to load data using table-valued functions (TVFs) such as [READ_CSV](/sql_reference/functions-reference/table-valued/read_csv), [READ_PARQUET](/sql_reference/functions-reference/table-valued/read_parquet), and [LIST_OBJECTS](/sql_reference/functions-reference/table-valued/list-objects).
...
You can use [DROP LOCATION](/sql_reference/commands/data-definition/drop-location) to remove a location from your Firebolt account. 
