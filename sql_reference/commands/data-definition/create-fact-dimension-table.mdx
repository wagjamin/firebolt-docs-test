...
| `<storage_parameter>`       | The name of a [storage parameter](/commands/data-definition#storage-parameters) for controlling behaviors related to storage and indexes. |
...
<Note>
Only literals and the following functions are supported in default expressions: [CURRENT_DATE](/sql_reference/functions-reference/date-and-time/current-date), [LOCALTIMESTAMP](/sql_reference/functions-reference/date-and-time/localtimestamp), [CURRENT_TIMESTAMP](/sql_reference/functions-reference/date-and-time/current-timestamptz), and NOW, which is an alias for CURRENT_TIMESTAMP.
</Note>
...
* [TO_YYYYMM](/sql_reference/functions-reference/date-and-time/to-yyyymm)
* [TO_YYYYMMDD](/sql_reference/functions-reference/date-and-time/to-yyyymmdd)
* [EXTRACT](/sql_reference/functions-reference/date-and-time/extract)`(year|month|day|hour from <column_name>)`
* [DATE_TRUNC](/sql_reference/functions-reference/date-and-time/date-trunc)

For more information, see [Working with partitions](/Overview/indexes/using-indexes#partitions-in-tables).

### Table type

Firebolt supports two types of [tables](/Overview/indexes/using-indexes#tables):
...
The [CREATE TABLE](/sql_reference/commands/data-definition/create-fact-dimension-table) command defaults to a `FACT` table. `DIMENSION` tables are ideal for relatively small tables, up to tens of gigabytes, that are used in joins with `FACT` tables.
...
| :-------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<index_granularity>` | The maximum number of rows in each granule. `<storage_parameter_value>` must be a power of 2 between 128 and 8192. The default value is 8192. For more information, see [Index granularity](/Overview/indexes/primary-index#advanced-option-index-granularity). |
| `<compression>` | The compression to use at the table level. The default value is `LZ4`. For more information, see [Custom Compression](/Overview/table-and-column-compression). |
| `<compression_level>` | The compression level to use  for the specidfied compression. This parameter cannot be set without `compression`. For more information, see [Custom Compression](/Overview/table-and-column-compression). |
...
* [CREATE TABLE AS SELECT (CTAS)](/sql_reference/commands/data-definition/create-fact-dimension-table-as-select) &ndash; - Creates a table and loads data into it based on a `SELECT` query. 
* [CREATE TABLE CLONE](/sql_reference/commands/data-definition) &ndash; Creates a table that is a copy of an existing table in the database.
