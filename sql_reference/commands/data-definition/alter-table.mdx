...
See the examples in [Working with partitions](/Overview/indexes/using-indexes#partitions-in-tables).
...
* Only for managed tables created on Firebolt version 4.10 or higher (external tables are not supported).
* The table must not have any dependent views.
* Renaming tables across schemas and databases is not supported. Consider using [CREATE TABLE CLONE](/commands/data-definition/create-table-clone)
