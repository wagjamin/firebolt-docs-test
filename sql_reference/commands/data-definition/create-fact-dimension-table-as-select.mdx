...
Creates a table and loads data into it based on the [SELECT](/commands/queries/select) query. The table column names and types are automatically inferred based on the output columns of the [SELECT](/commands/queries/select). When specifying explicit column names, those override the column names inferred from `SELECT`.
