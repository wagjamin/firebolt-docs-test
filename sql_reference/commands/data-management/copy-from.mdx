---
description: Reference and syntax for the COPY command that copies data from S3 files
  into a Firebolt table.
mode: wide
sidebarTitle: COPY FROM
title: COPY FROM
---
...
## Table of contents
* [Syntax](/commands/data-management#syntax)
* [Parameters](/commands/data-management#parameters)
* [Best practice](/commands/data-management#best-practice)
* [Examples](/commands/data-management#examples)

  **Data loading and filtering examples:**
    * [COPY FROM with LOCATION](/commands/data-management#copy-from-with-location)
    * [Filter by metadata during loading](/commands/data-management#filter-by-metadata-during-loading)
    * [Use LOCATION to copy data](/commands/data-management#use-location-to-copy-data)
    * [Load multiple directories with LOCATION](/commands/data-management#load-multiple-directories-with-location)
    * [Load multiple files and directories in parallel](/commands/data-management#load-multiple-files-and-directories-in-parallel)

  **Schema management examples:**
    * [Automatic schema discovery](/commands/data-management#automatic-schema-discovery)
    * [Use PATTERN to insert data into an existing table](/commands/data-management#use-pattern-to-insert-data-into-an-existing-table)
    * [Load metadata into a table](/commands/data-management#load-metadata-into-a-table)
    * [Column mapping](/commands/data-management#column-mapping)
    * [Allow column name mismatch](/commands/data-management#allow-column-name-mismatch)

  **Troubleshooting:**
   * [Error handling](/commands/data-management#error-handling)
...
| `location_name`               | The name of a location object that contains the S3 URL and credentials. This is the recommended approach for specifying the source. See [CREATE LOCATION](/sql_reference/commands/data-definition/create-location) for details.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `CREDENTIALS`                 | The Amazon S3 credentials for accessing the specified `<url>`. For more information, see [CREDENTIALS](/sql_reference/commands/data-definition/create-external-table#credentials).                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `ERROR_FILE_CREDENTIALS`      | The Amazon S3 credentials required to write an error file to an Amazon S3 URL. For more information, see [CREDENTIALS](/sql_reference/commands/data-definition/create-external-table#credentials).                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `DATE_FORMAT`         | Specify the date format for parsing text into date columns. This format will apply to all columns loaded as date columns. For supported formats, see [TO_DATE](/functions-reference/date-and-time/to-date).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `TIMESTAMP_FORMAT`    | Specify the timestamp format for parsing text into timestamp columns. The format will apply to all columns loaded as timestamp columns. For supported formats, see [TO_TIMESTAMP](/functions-reference/date-and-time/to-timestamp).                                                                                                                                                                                                                                                                                                                                                                   |
...
For detailed information, see [CREATE LOCATION](/sql_reference/commands/data-definition/create-location).
...
### Column mapping
When loading data into a target table, you can manually map source and target schemas by [column name](/commands/data-management#mapping-by-column-name) or [index position](/commands/data-management#mapping-by-index-position). Column mapping ensures that data from the source file is correctly inserted into the appropriate columns in the target table. 

##### Handling case sensitivity


By default, column mapping in Firebolt is **case-insensitive**, meaning column names in the `COPY FROM` statement are matched without quotes and are treated as lowercase. The [`CASE_SENSITIVE_COLUMN_MAPPING`](/commands/data-management#parameters) parameter is ignored, and data loads into the table regardless of case differences. If you need to enforce case-sensitive column mapping, set the `CASE_SENSITIVE_COLUMN_MAPPING` parameter to `TRUE`. When enabled, column names must match exactly, including case, and `COPY FROM` will either fail or populate columns with `NULL` values if there is a case mismatch. You can also use quoted identifiers to preserve case-sensitive behavior. 
...
Note that if a required column in the mapping is missing from the source file, the load will fail unless `ALLOW_COLUMN_MISMATCH` is enabled. For cases where column names may change or be missing in the source file, see the following section [`ALLOW_COLUMN_MISMATCH`](/commands/data-management#allow-column-name-mismatch). 
...
|------------------------------------|--------------------------|--------------------------|
| help_center_/assets/tournaments.csv | 1                        | Error while casting      |
