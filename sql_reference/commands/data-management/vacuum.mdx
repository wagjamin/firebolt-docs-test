...
`VACUUM` improves query efficiency by restructuring tablets for optimal performance. DML operations such as [DELETE](/commands/data-management/delete), [UPDATE](/commands/data-management/update), [INSERT](/commands/data-management/insert), and [COPY FROM](/commands/data-management/copy-from) might create tablets that are not optimally sized. Suboptimal tablets occur because DML efficiently utilizes resources in proportion to the cardinality of the data being inserted. In addition to standard SQL operations, tuples that are deleted by an update are not always physically removed from their table; they remain present until a `VACUUM` is finished operating. In other words, tablets are not necessarily optimal for running queries; therefore, it’s necessary to run `VACUUM` periodically, especially on frequently updated tables.
