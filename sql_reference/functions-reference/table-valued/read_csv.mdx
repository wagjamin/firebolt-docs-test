...
|:----------|:------------|:---------------------|
| LOCATION | The name of a location object that contains the Amazon S3 URL and credentials. Firebolt recommends using `LOCATION` to store credentials for authentication. See [CREATE LOCATION](/sql_reference/commands/data-definition/create-location) for details. | `IDENTIFIER` |
...
| `COMPRESSION`               | The [compression type](/sql_reference/commands/data-definition/create-external-table#compression) of the input file. If `compression` is not set, `compression` is inferred from the file extension.           | `TEXT`                |
