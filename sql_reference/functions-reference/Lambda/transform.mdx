...
| :----------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------- |
| `<function>` | A [Lambda function](/Guides/loading-data/working-with-semi-structured-data/working-with-arrays#manipulating-arrays-with-lambda-functions) used to check elements in the array. | Any Lambda function  |
...
In the example below, the `TRANSFORM` function is used to [CAST](/functions-reference/conditional-and-miscellaneous/cast) each element from a string to a date type. With each element now as a date type, the [INTERVAL](/Reference/interval-arithmetic) function is then used to add 5 years to each.  
