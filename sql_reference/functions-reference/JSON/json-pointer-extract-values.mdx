...
mode: wide
sidebarTitle: JSON_POINTER_EXTRACT_VALUES
...
| `<json_pointer_expression>` | A JSON pointer expression to the location of the desired sub-document in the JSON. For more information, see [JSON pointer expression syntax](/functions-reference/JSON#json-pointer-expression-syntax). | `TEXT`                |
...
see [JSON common example](/functions-reference/JSON#json-common-example). The **returned result** is based on the following example.
