...
mode: wide
sidebarTitle: APACHE_DATASKETCHES_HLL_BUILD
...
function [APACHE_DATASKETCHES_HLL_MERGE](/functions-reference/datasketches/apache-datasketches-hll-merge).
To estimate the final distinct count value from a sketch, the scalar
function [APACHE_DATASKETCHES_HLL_ESTIMATE](/functions-reference/datasketches/apache-datasketches-hll-estimate) can be
used.
