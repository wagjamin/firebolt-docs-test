...
`PERCENTILE_DISC` is available as a [window function](/functions-reference/window).
See also [PERCENTILE_CONT](/functions-reference/aggregation/percentile-cont), which calculates an interpolated result, rather than matching any of the specific column values.
