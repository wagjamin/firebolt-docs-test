...
mode: wide
sidebarTitle: REGEXP_EXTRACT
title: REGEXP_EXTRACT
---


 
Returns the first match of `<pattern>` within the `<expression>`. If the pattern does not match, returns `NULL`. If you want to extract all matches, use [REGEXP_EXTRACT_ALL](/functions-reference/string/regexp-extract-all).
