...
mode: wide
sidebarTitle: REGEXP_EXTRACT_ALL
title: REGEXP_EXTRACT_ALL
---

 
Returns an array that contains all matches of a `<pattern>` within the given `<expression>`. 
If the pattern does not match, returns an empty array. If you want return the first match of `<pattern>` within the `<expression>`, use [REGEXP_EXTRACT](/functions-reference/string/regexp-extract).
