...
The `DATE` function returns a result of type `DATE`. If the conversion cannot be performed, the `DATE` function raises an error. For more information on conversion rules, check out the documentation of the [DATE data type](/sql_reference/date-data-type).
