...
sidebarTitle: Asynchronous queries
...
- [INSERT](/reference-sql/commands/data-management/insert) &ndash; Inserts one or more values into a specified table.
- [COPY FROM](/reference-sql/commands/data-management/copy-from) &ndash; Loads data from an Amazon S3 bucket into Firebolt.
- [COPY TO](/reference-sql/commands/data-management/copy-to) &ndash; Copies the result of a `SELECT` query to an Amazon S3 location.
- [VACUUM](/reference-sql/commands/data-management/vacuum) &ndash; Optimizes tablets for query performance.
- [CREATE AGGREGATING INDEX](/reference-sql/commands/data-definition/create-aggregating-index) &ndash; Creates an index for precomputing and storing frequent aggregations.
- [CREATE AS SELECT](/reference-sql/commands/data-definition/create-fact-dimension-table-as-select) &ndash; Creates a table and loads data into it based on a `SELECT` query.
- [Engine commands](/reference-sql/commands/engines) including [ALTER ENGINE](/reference-sql/commands/engines/alter-engine), [STOP ENGINE](/reference-sql/commands/engines/stop-engine), and [START ENGINE](/reference-sql/commands/engines/start-engine). By default, Firebolt engines finish running queries before returning results, which can take significant time. Starting an engine can also take more than a few minutes.
...
* [Node.js](/guides/developing-with-firebolt/connecting-with-nodejs) &ndash; Firebolt Node SDK

## Submit a query

Submitting a query through a Firebolt drivers and SDKs have similar formats. The following code example shows how to submit an asynchronous query using the [Python SDK](/guides/developing-with-firebolt/connecting-with-Python). For other languages, consult the specific driver for details:
...
The query status token is included in the initial response when the query is submitted. If needed, you can also retrieve the token from the [engine_running_queries](/reference-sql/information-schema/engine-running-queries) view.
...
A running asynchronous query can be cancelled using the [CANCEL](/reference-sql/commands/queries/cancel) statement as follows:

```sql
CANCEL QUERY '<query_id>';
```

In the previous code example, retrieve the query ID from the [engine_running_queries](/reference-sql/information-schema/engine-running-queries) view or from the original query submission response.
