...
- **Firebolt database and engine** &ndash; You must have access to a Firebolt database. If you do not have access, you can [create a database](/Guides/getting-started/get-started-sql#create-a-database) and then [create an engine](/Guides/getting-started/get-started-sql#create-an-engine).
- **Firebolt service account** &ndash; You must have an active Firebolt [service account](/Guides/managing-your-organization/service-accounts) for programmatic access, along with its ID and secret.
...
4. Enter the connection parameters in the **Main** tab as follows:
   ...
   |-------------|-------------|
   | **JDBC URL** | Use `jdbc:firebolt:<db_name>?engine=<engine_name>&account=<account_name>` replacing `<db_name>` with your Firebolt [database name](/Overview/indexes/using-indexes#databases), `<engine_name>` with your [engine name](/Guides/getting-started/get-started-sql#create-an-engine) and `<account_name>` with your [account name](/Guides/managing-your-organization/managing-accounts). |
...
* Discover other tools that [Firebolt integrates](/Guides/integrations/integrations) with.
