---
description: Learn how to navigate and use the develop space to work with your Firebolt
  database.
mode: wide
sidebarTitle: Use the Develop Space
...
1. [Explore panel](/query-data#using-the-explore-panel): Panel on the **left** that displays available databases, engines, and security users and roles. 
2. [Document editor](/query-data#using-the-document-editor): Panel in the **center** that provides tools to edit, run, and manage your SQL scripts organized as tabs.
3. [Results panel](/query-data#using-the-results-panel): Panel at the **bottom** that shows results, execution statistics, engine monitoring information, and query history.
...
* [Format a script](/query-data#format-a-script)
* [Renaming a script](/query-data#renaming-a-script)
* [Using script templates](/query-data#using-script-templates)
* [Delete a script](/query-data#delete-a-script)
* [Run a script and view results](/query-data#run-a-script-and-view-results)

Choose the vertical ellipsis (**⋮**) next to the script name in the left pane to [format](/query-data#format-a-script), [rename](/query-data#renaming-a-script),
...
At the top of each script tab, you can choose **Run** to run your SQL statements. SQL statements can only run on running engines. If an engine isn't running, you can select it from the list and then choose the **Start** button for that engine. For more information about engines, see [Operate engines](/Guides/operate-engines/operate-engines).
...
* [Query operations](/query-data#query-operations)
* [Script management](/query-data#script-management)
* [Search functionality](/query-data#search-functionality)
* [Editing text](/query-data#editing-text)
