...
mode: wide
sidebarTitle: Load data using a wizard
...
2. Select an engine size from the drop-down list next to **Node type**. Consider the following when creating a new engine:
    ...
    1. If you are loading data and using Firebolt for the first time, use the smallest engine size (S) and a small dataset to try out Firebolt’s capabilities. Refer to the [Get Started](/getting-started) guide for more information.
    ...
    Small and medium engines are available for use right away. If you want to use a large or extra-large engine, reach out to support@firebolt.io. For more information, see [Sizing Engines](/Guides/operate-engines/sizing-engines).
...
3. For authentication:
   ...
   - For more information about these credentials, see [Create Access Key and Secret ID in AWS](/loading-data/creating-access-keys-aws).
4. Select **Next Step**.
...
`s3://firebolt-publishing-public/help_center_/assets/firebolt_sample_dataset/`. 
...
1. **Type** - you can change the [data type](/sql_reference/data-types) of the column.
