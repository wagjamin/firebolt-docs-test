...
* [Role-Based Access Control](/Overview/Security/Role-Based Access Control) &ndash; How administrators manage user permissions and control access to resources based on predefined roles.
...
To view all roles using SQL, query the [information_schema.applicable_roles](/sql_reference/information-schema/applicable-roles) view as shown in the following code example:
...
The following code example uses [CREATE ROLE](/sql_reference/commands/access-control/create-role) to create the role `user_role`:
...
To delete a role using SQL, use [DROP ROLE](/sql_reference/commands/access-control/drop-role) as shown in the following code example:
...
To grant a permission to a role using SQL, use [GRANT](/sql_reference/commands/access-control/grant) as shown in the following code example:
...
To grant a role to a user or another role using SQL, use [GRANT ROLE](/sql_reference/commands/access-control/grant) as shown in the following code example:
...
To revoke a permission from a role using SQL, use [REVOKE](/sql_reference/commands/access-control/revoke) as shown in the following example:
...
To revoke permissions, follow the same steps described in [Grant permissions to a role](/security#grant-permissions-to-a-role).
...
To revoke a role from a user or another role using SQL, use the [REVOKE ROLE](/sql_reference/commands/access-control/revoke) statement. For example:
...
To revoke a role, follow the steps in [Grant a role to users](/security#grant-a-role-to-users).
