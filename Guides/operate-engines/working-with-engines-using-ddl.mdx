...
* [Create engines](/operate-engines#create-engines) &ndash; Learn how to create an engine.
* [Start or resume an engine](/operate-engines#start-or-resume-an-engine) &ndash; Learn how to start or resume an engine.
* [Stop an engine](/operate-engines#stop-an-engine) &ndash; Learn how to stop an engine either gracefully or immediately.
* [Resize engines](/operate-engines#resize-engines) &ndash; Learn how to scale engines up or down by adjusting the node type or number of nodes.
* [Concurrency auto-scaling](/operate-engines#concurrency-auto-scaling) &ndash; Learn how to enable auto-scaling for engines to automatically adjust the number of clusters based on workload.
* [Automatically start or stop an engine](/operate-engines#automatically-start-or-stop-an-engine) &ndash; Learn how to configure engines to start and stop automatically based on specific conditions.
...
To create an engine, use [CREATE ENGINE](/sql_reference/commands/engines/create-engine).
...
To start your engine, use the [START ENGINE](/sql_reference/commands/engines/start-engine) command:
...
To stop an engine, use the [STOP ENGINE](/sql_reference/commands/engines/stop-engine) command:
...
Use the [ALTER ENGINE](/sql_reference/commands/engines/alter-engine) command to change the node type:
...
Use the [ALTER ENGINE](/sql_reference/commands/engines/alter-engine) command to change the number of nodes:
...
<Note>
If you are using Firebolt in preview mode, you can only use a single cluster for your engines. If you want to try using multi-cluster engines, contact [Firebolt support](mailto:support@firebolt.io). Additionally, when scaling an engine, both the old and new compute resources may be active at the same time for a period. This simultaneous operation can result in higher consumption of Firebolt Units ([FBUs](/Overview/engine-consumption)). 
</Note>
...
<img src="/assets/images/Engine_Initially_Stopped.png" alt="" width="600"/><br/>
...
<img src="/assets/images/Engine_Auto_Stop.png" alt="" width="600"/> <br/> 

### Configure automatic start/stop using the API
Use the [CREATE ENGINE](/sql_reference/commands/engines/create-engine) command to set auto-start and auto-stop options:
...
To modify the auto-stop feature later, use the [ALTER ENGINE](/sql_reference/commands/engines/alter-engine) command: 
