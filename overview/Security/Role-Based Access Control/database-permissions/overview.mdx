...
| [VACUUM](/sql_reference/commands/data-management/vacuum.md ) ANY             | Allows running the `VACUUM` operation on all current and future tables.              | `GRANT VACUUM ANY ON DATABASE <database_name> TO <role>;`              | `REVOKE VACUUM ANY ON DATABASE <database_name> FROM <role>;`           |
...
### USAGE permission
The following code example [grants](/sql_reference/commands/access-control/grant.md ) the role `developer_role` access to use the specified database:
...
The following code example gives the role `developer_role` permission to run [VACUUM](/sql_reference/commands/data-management/vacuum.md ) operations on all current and future tables in the specified database:
