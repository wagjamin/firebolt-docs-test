...
In the following code example, a user `test_user` with an [account_admin](/Overview/organizations-accounts.md #account-administrative-role) role retrieves their privileges and associated roles for their current user session:
