---
sidebarTitle: Engine Fundamentals
...
* [Engine governance and security](#engine-governance-and-security) &ndash; Control engine access using [Role-Based Access Control (RBAC)](/Overview/Security/Role-Based Access Control/index.md ) and account-level isolation to enforce security policies.
...
<div className="text-center">
  <img src="/assets/images/engine_cluster_type_M.png" alt="/1" width="600" className="mx-auto" />
  <p className="text-sm text-gray-500 mt-2">An engine cluster with four nodes of type 'M'</p>
</div><div className="text-center">
  <img src="/assets/images/Engine_two_clusters_type_M.png" alt="/1" width="600" className="mx-auto" />
  <p className="text-sm text-gray-500 mt-2">A Firebolt engine with two clusters, each cluster containing four nodes of type 'M'</p>
</div>
...
For a full list of engine attributes, see [CREATE ENGINE](../sql_reference/commands/engines/create-engine)
...
Use the [ALTER ENGINE](/sql_reference/commands/engines/alter-engine.md ) to modify the configuration of an engine to dynamically scale the engine even while it is running, without impacting the workload. 
...
* An engine URL.
* Third-party [connectors](/Guides/integrations/integrations.md ) such as Airflow and DBT.
...
You can use the information in the previous [information_schema views](/sql_reference/information-schema/index.md ) to decide whether you need to change the engine configuration type, number of nodes or clusters based on your workload needs.
...
The Firebolt [RBAC model](/Guides/security/rbac.md ) allows administrators to control user actions on resources that are created within a given account. For example, administrators can control which users are allowed to modify the configuration of engines and control which users can create new engines. 
...
Use [SHOW ENGINES](/sql_reference/commands/metadata/show-engines.md ) to list all engines and their statuses in your Firebolt account as follows:
